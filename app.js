(function () {
  ...
  document.getElementById("zfit").onclick = () => onMap() ? fitMap(true) : fit();
  ...
  function paint() {
    ...
    if (state.tab === "time") { renderTimeline(); return; }
    tally();
  }
  ...
  function renderSummary() {
    ...
    const lives = nodes.filter(n => n.type === "person" && n.span);
    const first = Math.min.apply(null, lives.map(n => n.span[0]));
    const last = Math.max.apply(null, lives.map(n => n.span[1]));

    detail.innerHTML = '<div class="panel">' +
      '<div class="sect"><h2>What you are looking at</h2>' +
        '<p class="pnote">Manchester and Salford, through people at work between ' +
        first + ' and ' + last + ': ' +
  ...
  const mzoom = d3.zoom().scaleExtent([0.12, 16]).on("zoom", ev => {
    mk = ev.transform.k;
  ...
  function fitMap(animate) {
    ...
    mapFitted = true;
    if (animate) msvg.transition().duration(400).call(mzoom.transform, t);
    else msvg.call(mzoom.transform, t);
  }
  ...
  /* ---------------- the timeline ----------------
     One row per record, its bar running from the first year it was active to the
     last. Events are single years, so they draw as a point. Nothing here is new
     data: it is the same spans the decade slider uses, laid out to be read. */
  const tl = document.getElementById("timeline");
  const TL0 = 1750, TL1 = 1960;
  const tlx = y => ((Math.max(TL0, Math.min(TL1, y)) - TL0) / (TL1 - TL0)) * 100;

  const TL_GROUPS = [
    ["event", "Events"], ["person", "People"],
    ["org", "Societies, firms and groups"], ["place", "Places"]
  ];

  function renderTimeline() {
    const ticks = [];
    for (let y = TL0; y <= TL1; y += 10) {
      ticks.push('<i style="left:' + tlx(y) + '%"></i>' +
        (y % 20 === 0 && y <= TL1 - 20 ? '<span style="left:' + tlx(y) + '%">' + y +
          '</span>' : ""));
    }
    let html = '<div class="tlaxis"><b>When and who</b><div class="tlticks">' +
      ticks.join("") + '</div></div>';

    let shown = 0;
    TL_GROUPS.forEach(g => {
      const rows = nodes.filter(n => n.type === g[0] && visibleNode(n) && n.span)
        .sort((p, q) => p.span[0] - q.span[0] || p.label.localeCompare(q.label));
      if (!rows.length) return;
      shown += rows.length;
      html += '<div class="tlgroup">' + esc(g[1]) + ' &middot; ' + rows.length + '</div>';
      html += rows.map(n => {
        const x0 = tlx(n.span[0]), x1 = tlx(n.span[1]);
        const point = n.span[1] - n.span[0] < 2;
        const est = n.spanSource === "estimated" || n.spanSource === "inferred";
        const clip = (n.span[0] < TL0 ? " clipL" : "") + (n.span[1] > TL1 ? " clipR" : "");
        const bar = '<b class="' + (point ? "point" : "") + (est ? " est" : "") + clip +
          '" style="left:' + x0 + '%;' +
          (point ? "" : "width:" + Math.max(x1 - x0, 0.35) + "%;") +
          'background:' + cssvar(n.domain) + '"></b>';
        const yr = n.span[0] === n.span[1] ? String(n.span[0])
          : n.span[0] + "\u2013" + String(n.span[1]).slice(2);
        return '<button class="tlrow' + (n.id === state.selected ? " on" : "") +
          '" data-go="' + esc(n.id) + '" title="' + esc(n.label) + '">' +
          '<span class="yr">' + yr + '</span>' +
          '<span class="who">' + esc(n.label) + '</span>' +
          '<span class="tltrack">' + bar + '</span></button>';
      }).join("");
    });

    if (!shown) {
      html += '<p class="tlnote">Nothing is in view. Widen the filters on the left.</p>';
    } else {
      html += '<p class="tlnote">Events are single years and draw as a dot; everything ' +
        'else is a bar from the first year it was active to the last. A pale bar is a ' +
        'span worked out from a lifespan, or from what a group connects to, rather than ' +
        'a date the collection states \u2014 read those as an era, not a record. The axis ' +
        'runs 1750 to 1960; a few buildings are older than that and their bars are ' +
        'squared off at the end where they run past it. Click any row to open it.</p>';
    }

    tl.innerHTML = html;
    const vis = nodes.filter(n => visibleNode(n) && n.span);
    document.getElementById("t-nodes").textContent = shown;
    document.getElementById("t-edges").textContent =
      vis.filter(n => n.type === "event").length;
    document.getElementById("t-cross").textContent =
      vis.filter(n => n.type === "person").length;
    document.getElementById("t-iso").textContent =
      vis.filter(n => n.type === "place").length;
    document.getElementById("t-todo").textContent =
      vis.filter(n => n.spanSource !== "workbook").length;
  }
  ...
  grid.addEventListener("click", e => {
    ...
    if (tr) select(tr.getAttribute("data-go"));
  });
  ...
  const COLS = {
    person: [
      ...
      ["dates", "Dates", "narrow", "text"], ["gender", "Gender", "tiny", "text"],
  ...
  function cellText(row, c) {
    ...
    if (kind.indexOf("select:") === 0) {
      ...
      return opts[v] ? opts[v].label : String(v);
    }
    return String(v);
  }
  ...
  function renderSheet() {
    ...
    const head = '<thead><tr>' + cols.map(c => '<th>' + esc(c[1]) + '</th>').join("") +
      '<th>' + (isLinks ? "Rel" : "Record") + '</th></tr></thead>';

    const body = '<tbody>' + rows.map(r => {
      const cls = (!isLinks && has(r, "qcFlag")) ? "flagged" : "";
      const open = isLinks ? nid(r.source) : r.id;
      return '<tr class="' + cls + '" data-go="' + esc(open) + '">' +
        cols.map(c => '<td class="' + c[2] + '"><span class="cell">' +
          esc(cellText(r, c)) + '</span></td>').join("") +
        '<td class="idcell">' + esc(isLinks ? (r.relId || "\u2014") : r.id) + '</td></tr>';
    }).join("") + '</tbody>';
